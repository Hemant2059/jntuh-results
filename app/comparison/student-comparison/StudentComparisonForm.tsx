...
export default function StudentComparisonForm() {
  ...
  // Track loading for each hallticket
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      const fetchPromises = halltickets.map(async (hallticket) => {
        ...
        setLoadingTickets((prev) => ({ ...prev, [hallticket]: true }));
        ...
        setLoadingTickets((prev) => ({ ...prev, [hallticket]: false }));
      });
  ...
  const { subjects, formattedData }: FormattedData =
    resultData.length > 0 ? formatStudentData(resultData) : { subjects: [], formattedData: [] };
  ...
  const loadingMessages = useMemo(() => {
    return Object.keys(loadingTickets)
      .filter((ticket) => loadingTickets[ticket])
      .map((ticket) => (
  ...
  return (
    <div>
      {loading ? (
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="grid gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    onValueChange={(value) => setSemester(value as keyof typeof SemesterData)}
                    name="semester"
                  >
                    <SelectTrigger>
                ...
                <div className="text-xs text-red-500">
                  Note: Enter the last student&apos;s number like <strong>20XXXXXXA9</strong> then <strong>A9</strong> otherwise <strong>0</strong>
                </div>
                {/* Submit Button */}
        ...
        <div className="container mx-auto py-10 md:px-2">
          <div className="md:p-4">
            <div className="mb-2 border-b-2 text-center text-[45%] md:text-[60%] lg:text-[100%]">
              <p className="font-semibold">{collegeName}</p>
              <p className="font-semibold">{courseName}</p>
              <p className="font-semibold">
                {semester ? SemesterData[semester] : "Unknown Semester"}
              </p>
            </div>
            <div>
              <StudentComparisonTable students={formattedData} />
            </div>
          </div>
        </div>
      )}
